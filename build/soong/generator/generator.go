...
func init() {
	android.RegisterModuleType("aosuip_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = aosuipExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = aosuipExpandVariables(ctx, dep_file)
	...
	cmd := aosuipExpandVariables(ctx, String(g.properties.Cmd))
